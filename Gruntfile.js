module.exports = function(grunt) {
    grunt.initConfig({
        ...
        copy: {
          main:{
            files:[
              {src: ['app/favicon.ico'], dest: 'build/favicon.ico', flatten: true,filter: 'isFile'},
              {src: ['app/index.tpl'], dest: 'build/index.html', flatten: true},
              {expand: true,  src: ['app/common/*.html'], dest: 'build/common/', flatten: true},
              {expand: true,  src: ['app/pprof/*.html'], dest: 'build/pprof/', flatten: true},
              {expand: true,  src: ['app/setting/*.html'], dest: 'build/setting/', flatten: true},
              {expand: true,  src: ['app/stats/*.html'], dest: 'build/stats/', flatten: true},
              {expand: true,  src: ['app/bower_components/datatables/media/images/*'], dest: 'build/images/', flatten: true},
            ],
          },
        },
    });
